...
public sealed class QueryInLoopRule : IRule
{
    ...
    public IEnumerable<Diagnosis> Analyze(QueryShapeScope scope)
    {
        ...
        var groups = RuleHelpers.ReadQueries(scope)
            ...
            .GroupBy(c => (c.CallSite!.FilePath ?? string.Empty) + "|" + c.CallSite.Member, StringComparer.Ordinal);
        ...
        foreach (var group in groups)
        {
            ...
            yield return new Diagnosis(
                RuleId,
                DefaultSeverity,
                $"Queries in a loop: {site.Member} issued {commands.Count} queries of {shapes.Count} shapes ({string.Join(", ", roots)}) in one scope",
                $"{site.Member}{lineText} ran {commands.Count} queries in this scope, with {shapes.Count} different shapes each repeated at least {repeatsPerShape} times. " +
                ...
                "Unlike a plain N+1 (QS001), the per-iteration queries here are of different shapes, so a single Include will not fix all of them.",
                site,
                ...
                new Fix(
                    $"Load the data {site.Member} needs for all items before the loop (Include/Select on the outer query, or one batched query per shape keyed by the item ids), then pass it in",
                    FixKind.CodeChange,
                    null,
                    null,
                    null,
