...
The current corpus covers four external application repositories, plus deliberately bad/fixed sample operations and provider integration tests. External datasets are synthetic. Passing tests are evidence for those executions, not a production guarantee or an adoption study.
...
[Hosted application run on 2026-09-11](https://github.com/DanilAntyp/QueryShape/actions/runs/34550615354): all 33 application tests passed against QueryShape commit `f768a6e` — 15 eShopOnWeb and 4 Ombi on native .NET 8, 2 CleanArchitecture and 12 Jellyfin on .NET 10, the last including five that drive Jellyfin's real HTTP API through its own server host. All four jobs uploaded their scope reports and TRX results. ([Earlier: 2026-09-10 without the API tests](https://github.com/DanilAntyp/QueryShape/actions/runs/34548187494), [2026-09-08 without Jellyfin](https://github.com/DanilAntyp/QueryShape/actions/runs/34278103529).)
...
| [Jellyfin results](jellyfin.md), pinned `cf09de60e4e5844ad181d7ef9019151c54969d44` | Library page, latest items, by-name and id-only paths through the real `BaseItemRepository`; row multiplication of five collection includes; upstream's own split-query branch as the measured alternative. Plus its real HTTP API in its own test host: request scopes, the response header, OpenTelemetry span tags and events, and QS002/QS008/QS012 on the authentication path | EF10/SQLite native, no mapping adaptation; synthetic 24-movie library with a fixed per-item fan-out; the flagged pattern is deliberate upstream (EF's own warning is suppressed), so it is a measurement, not a bug report |
| [Ombi results](ombi.md), pinned `c56440986e06012fcb57b66c3b892b94b03040a5` | Plex content repository: two collection includes on one entity, tracked reads, count and existence checks | EF8/SQLite native, real upstream migrations; synthetic two-show library; the repository is called directly, not the sync job that uses it |
...
| SQL Server, PostgreSQL, MySQL and Oracle provider tests | Provider shapes, expression correlation, row counts, QS001 and QS004 on each dialect | Require Docker; skipped locally and on Windows CI. Oracle runs on request (`QUERYSHAPE_ORACLE=1`, [green on 2026-09-10](https://github.com/DanilAntyp/QueryShape/actions/runs/34498861141)); the other three run on every push. [What each verifies](../providers.md) |
...
- Validate more applications and production-representative datasets. Record actionable findings, intentional patterns flagged, missed issues, integration effort and confirmed improvements. Four applications do not establish precision/recall or typical adoption effort.
