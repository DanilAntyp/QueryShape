...
| QS012 | Synchronous query on an async path | Command executed through EF Core's synchronous API inside a scope that declared itself asynchronous (`host.async`, set by `app.UseQueryShape()`); blocks a thread-pool thread for the duration of the call (added 2026-09-10, ADR-0015) | Warning |
...
- [x] Benchmarks recorded (`docs/performance.md`): ≈ 3.5 µs per query (micro) and, under 8-way concurrent load through the sample app, p99 unchanged on the 41-query request and within noise (+6 % of a 0.6 ms request) on the single-query one. Provider benchmarks ran on 2026-09-10 against PostgreSQL 16 and SQL Server 2022 containers: capture costs 19–40 µs and ≈ 4 KB per two-query read, analysis a further 7–25 µs, i.e. a few percent of a ~1 ms provider round trip. These workloads do not establish a universal overhead bound, and the percentage moved between two runs of the same commit; the 3% p99 figure remains a target, not a measurement.
